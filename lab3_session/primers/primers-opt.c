...
typedef struct {
    ...
    int shiftVal; /* for better bit arithmetic */
...
BITARRAY *createBitArray(bignum bits)
{
    ...
    ba->shiftVal = log2(ba->bitsPerInt);
    ba->bytesPerInt = sizeof(unsigned int);
    ba->bitsInArray = bits;
    ba->intsInArray = (bits >> ba->shiftVal) + 1; // udivdi3
...
{
    unsigned int *pInt = ba->p + (bitSS >> ba->shiftVal); // udivdi3
    unsigned int remainder = (bitSS & (ba->bitsPerInt - 1)); // umoddi3
    *pInt |= (1 << remainder);
}
