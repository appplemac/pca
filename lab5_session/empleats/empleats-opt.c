...
typedef struct {
  unsigned int NID;
  unsigned int empleat_index;
} index_node;

int compare_index(const void *p1, const void *p2) {
    if (((index_node *) p1)->NID > ((index_node *) p2)->NID) return(1);
    else if (((index_node *) p1)->NID < ((index_node *) p2)->NID) return(-1);
    else return(0);
}
...
int main(int argc, char *argv[])
{
  ...
  index_node * index = (index_node *)malloc(sizeof(index_node) * N);

	/* Random generation */
	for (i=0; i<N; i++) {
		empleats[i].NID = rand();
		index[i].NID = empleats[i].NID;
    index[i].empleat_index = i;
